...
object Create {
  ...
  val payloadString = randomAlphanumericString(TestConfig.testParamPayloadSize)


  val sb = new StringBuilder();

  for (i <- 1 to TestConfig.testParamNumPayloads) {
    sb.append("\"property").append(i).append("\":\"").append(payloadString).append(i).append("\"");
    if(i < TestConfig.testParamNumPayloads) {
      sb.append(",");
    }
  }
  ...
  val push = exec(session => {
    ...
    exec(http("Push new Document Revision")
      ...
      .check(jsonPath("$..rev").saveAs("currentrev"))
    ).pause(TestConfig.testParamSleepTimeMs milliseconds)
  }
}
